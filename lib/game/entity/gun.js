	...
	Gun = MapEntity.extend({
		...
		detect: function() {
			...
			if (ig.game.man.tile[this.axis] === this.tile[this.axis]) {
				...
				if (check) {
					var vx = (this.axis === 'x') ? 2: DISTANCE * ig.global.TILE_SIZE;
					var vy = (this.axis === 'y') ? 2: DISTANCE * ig.global.TILE_SIZE;
					var res = ig.game.collisionMap.trace(
						this.gunPos.x, this.gunPos.y, vx, vy, 2, 2
					);
					ig.log('res.collision', res.collision);
		...
		shoot: function() {
			...
			ig.game.spawnEntity('Lazer', this.gunPos.x, this.gunPos.y,
				{
