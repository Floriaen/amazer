...
.defines(function(){
	...
	Man = MapEntity.extend({
		...
		spawnSfx: new ig.Sound('media/sfx/RESPAWN_1.*', false),
		...
		goByTile: function(x, y) {
			...
			if (ig.game.collisionMap.data[ty][tx] === 0) {
				...
				/*
				// pickup step sound randomly
				var sfx = Man.walkSfxs[Math.floor(Math.random() * 2)];
				//sfx.volume = 0.3;
		...
		update: function() {
			...
			if (this.isCurrentAnim('rising')) {
				if (!this.spawnSfx.currentClip || this.spawnSfx.currentClip.ended) {
	...
	Man.walkSfxs = [
		new ig.Sound('media/sfx/WALK_3.*', false)
	];
