	...
	Map = ig.Entity.extend({
		...
		loadLevel: function(level) {
			...
			for (var i = 0; i < level.length; i++) {
				...
				var tileset = level[i].tilesetName || TILESET_MAZE;
				var floor = new MapFloor(
					ig.global.TILE_SIZE,
					level[i]['layer'][0]['data'],
					tileset
				);
				floor.name =  level[i].name || 'Level ';
		...
		goFloor: function(level) {
			if (this.z + level > 0 && this.z + level < this.currentFloor.length) {
