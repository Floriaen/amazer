...
.defines(function(){
    ...
    Map = ig.Entity.extend({
		...
		loadLevel: function(level) {
			...
			//var maps = level['layer'][0]['data'];
			for (var i = 0; i < level.length; i++) {
				if (level[i]['entities']) {
					this.mapEntities.push(level[i]['entities']);
				} else {
		...
		loadCurrentMap: function() {
			// in case we restart:
			this.floor.z = this.z;
			this.floor.start.x = ig.game.man.tile.x * ig.global.TILE_SIZE;
			this.floor.start.y = ig.game.man.tile.y * ig.global.TILE_SIZE;
			...
			if (!this.entities[this.z]) {
				...
				if (mapEntities) {
					for (i = 0; i < mapEntities.length; i++) {
						...
						if (mapEntity.settings && mapEntity.settings.name) {
							entity.name = mapEntity.settings.name;
						}
				...
				for (i = 0; i < map.data.length; i++) {
					for (var j = 0; j < map.data[i].length; j++) {
						...
						if (value === TILE) {
							...
							entityClassName = 'Gun';
							map.data[i][j] = 1;
						} else if (value === FALLING_BAG) {
							entityClassName = 'Maggot';
							//entityClassName = 'Spikes';
							//entityClassName = 'Bag';
		...
		setTileValue: function(tile, value) {
			var map = this.maps[this.z].data;
			if (map && tile.x >= 0 && tile.y >= 0 && tile.y < map.length && tile.x < map[tile.y].length) {
				this.maps[this.z].data[tile.y][tile.x] = value;
			}
			this.maps[this.z].preRenderedChunks = null; // force the redraw
		},
