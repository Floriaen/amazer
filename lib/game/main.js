...
.defines(function(){
	...
	AmazerGame = ig.Game.extend({
		...
		init: function() {
			...
			ig.input.bind(ig.KEY.MOUSE1, "TOUCH_PAUSE");
...
/*
		...
		update: function() {
			...
			if (ig.input.pressed("PAUSE") || ig.input.pressed("TOUCH_PAUSE")) {
	...
	function _showPause() {
		...
		ig.system.context.font = "48px Impact";
		ig.system.context.textAlign = 'center';
		ig.system.context.fillText(
			'PAUSE',
			(ig.system.width * 0.5) * ig.system.scale,
			(ig.system.height * 0.5) * ig.system.scale
		);
	}

	var size = 7 * ig.global.TILE_SIZE * 4;//window.innerHeight;
