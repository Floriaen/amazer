...
.defines(function(){
	...
	ig.global.currentLevel = 8;
	ig.global.currentFloor = 0;
	...
	AmazerGame = ig.Game.extend({
		...
		init: function() {
			...
			this.gameLogoImage = new ig.Image("media/amazer-logo.png");
		...
		restart: function() {
			this.restarting = false;
			ig.game.man = ig.game.spawnEntity('Man', this.map.floor.start.x, this.map.floor.start.y);
		},
		...
		update: function() {
			...
			if (this.overlay < this.map.z / ig.global.MAX_FLOOR) this.overlay = this.map.z / ig.global.MAX_FLOOR;

			if (this.showMenu) {
				if (this.overlay < 0.5) this.overlay = 0.5;
				if (ig.game.man && ig.game.man.vel.x > 0 || ig.game.man.vel.y > 0) {
					this.showMenu = false;
				}
			} else {
		...
		draw: function() {
			...
			if (this.showMenu) {
				...
				ig.global.DEFAULT_FONT_16.draw(
					'Level ' + ig.global.currentLevel + ' : ' + this.map.z,
					16, 100, ig.Font.ALIGN.LEFT
				);
			}
		}
	});
	...
	function _showMenu() {
		...
		ig.game.gameLogoImage.draw((ig.system.width - ig.game.gameLogoImage.width) / 2, 12)
