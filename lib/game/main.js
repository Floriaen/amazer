...
.defines(function(){
	...
	Cube = ig.Game.extend({
		...
		init: function() {
			...
			this.map = this.spawnEntity(
				'Map', 0, 0//(ig.system.width / (2 * ig.system.scale)).floor(), (ig.system.height / (2 * ig.system.scale)).floor()
			);
...
/*
		...
		levelUp: function() {
			if (this.level > ig.global.LEVEL_COUNT - 1) this.level = 0; // TODO for debug purpose
			this.map.loadLevel(ig.copy(window['level' + ++this.level]));
			//this.map.loadLevel(window['level' + ++this.level]);
		},
		...
		draw: function() {
			...
			var dx = ig.game.man.pos.x;
			this.transform[1] = 0;//(0.5 - dx / (ig.game.man.pos.x)) * ig.system.scale * 0.02;
			...
			this.transform[5] = 0;//- ig.game.man.pos.x * ig.system.scale * this.transform[1] * 1.5;

		//	ig.system.context.translate(100, 100);
